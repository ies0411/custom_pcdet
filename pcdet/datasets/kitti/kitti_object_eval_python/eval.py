...
def clean_data(all_classes, gt_anno, dt_anno, current_class, difficulty):
...
def _prepare_data(all_classes, gt_annos, dt_annos, current_class, difficulty):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        rets = clean_data(
            all_classes, gt_annos[i], dt_annos[i], current_class, difficulty
        )
...
def eval_class(
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(
                all_classes, gt_annos, dt_annos, current_class, difficulty
            )
            (
                gt_datas_list,
                dt_datas_list,
...
def do_eval(
    ...
    ret = eval_class(
        all_classes, gt_annos, dt_annos, current_classes, difficultys, 1, min_overlaps
    )
    ...
    ret = eval_class(
        all_classes, gt_annos, dt_annos, current_classes, difficultys, 2, min_overlaps
    )
...
def get_custom_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    """
    metric: eval type. 0: bbox, 1: bev, 2: 3d
    min_overlaps: float, min overlap. format: [num_overlap, metric, class].
    """
    ...
    overlap_0_7 = 0.7 * np.ones(shape=(3, class_n), dtype=np.float32)
    overlap_0_5 = 0.5 * np.ones(shape=(3, class_n), dtype=np.float32)
    ...
    all_classes = copy.deepcopy(current_classes)
